...
export async function bootstrap() {
  console.log('[SmartReply Bootstrap] 正在初始化 Puppeteer...')
  const { puppeteer } = await initPuppeteer()
  console.log('[SmartReply Bootstrap] Puppeteer 初始化完成')

  console.log('[SmartReply Bootstrap] 正在启动浏览器...')
  console.log('[SmartReply Bootstrap] PUPPETEER_EXECUTABLE_PATH:', process.env.PUPPETEER_EXECUTABLE_PATH)
  ...
  console.log('[SmartReply Bootstrap] 浏览器已启动')
  return browser
}

export async function launchBoss(browser: Browser, skipCookies = false) {
  console.log('[SmartReply LaunchBoss] 正在准备页面...')
  ...
  if (!skipCookies) {
    ...
    if (bossCookies && bossCookies.length > 0) {
      console.log('[SmartReply LaunchBoss] 设置 cookies, 数量:', bossCookies.length)
      for (let i = 0; i < bossCookies.length; i++) {
        await page.setCookie(bossCookies[i])
      }
    } else {
      console.log('[SmartReply LaunchBoss] 没有有效的 cookies')
    }
    if (bossLocalStorage && Object.keys(bossLocalStorage).length > 0) {
      console.log('[SmartReply LaunchBoss] 设置 localStorage')
      await setDomainLocalStorage(browser, localStoragePageUrl, bossLocalStorage)
    }
  }

  const url = getChatUiUrl()
  console.log('[SmartReply LaunchBoss] 正在导航到:', url)

  try {
    await page.goto(url, { timeout: 120 * 1000, waitUntil: 'domcontentloaded' })
    console.log('[SmartReply LaunchBoss] 页面加载完成')
  } catch (error: any) {
    console.error('[SmartReply LaunchBoss] 导航失败:', error)
