...
function getChatUiUrl() {
  const configuredUrl = String(readConfigFile('boss.json')?.recruiterAutoReply?.chatUiUrl ?? '')
    .trim()
  if (configuredUrl) {
...
export async function bootstrap() {
  console.log('[bootstrap] 正在初始化 Puppeteer...')
  const { puppeteer } = await initPuppeteer()
  console.log('[bootstrap] Puppeteer 初始化完成')

  console.log('[bootstrap] 正在启动浏览器...')
  console.log('[bootstrap] PUPPETEER_EXECUTABLE_PATH:', process.env.PUPPETEER_EXECUTABLE_PATH)
...
export async function launchBoss(browser: Browser, skipCookies = false) {
  ...
  if (!skipCookies) {
    ...
    if (bossCookies && bossCookies.length > 0) {
      console.log('[launchBoss] 设置 cookies, 数量:', bossCookies.length)
      for (let i = 0; i < bossCookies.length; i++) {
        await page.setCookie(bossCookies[i])
      }
    } else {
      console.log('[launchBoss] 没有有效的 cookies')
    }
    if (bossLocalStorage && Object.keys(bossLocalStorage).length > 0) {
      console.log('[launchBoss] 设置 localStorage')
      await setDomainLocalStorage(browser, localStoragePageUrl, bossLocalStorage)
    }
  }

  const url = getChatUiUrl()
  console.log('[launchBoss] 正在导航到:', url)

  try {
    await page.goto(url, { timeout: 120 * 1000, waitUntil: 'domcontentloaded' })
    console.log('[launchBoss] 页面加载完成')
  } catch (error: any) {
    console.error('[launchBoss] 导航失败:', error)
